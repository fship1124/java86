...
public class AuthFilter implements Filter {
	...
	public void init(FilterConfig config) throws ServletException {
		...
		noCheckPages = config.getInitParameter("noCheckPages").trim();
...
//		System.out.println(pages);
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		//   - 세션에 account 정보가 등록되어 있는지 확인한다.
		//     account 정보가 등록되지 않은 경우는 loginForm 페이지로 이동시킴
		//     account 정보가 등록되어 있는 경우 다음 페이지로 이동시킴
		...
		if (requestUri.startsWith(noCheckPages)) isRedirect = false;
		
		if (isRedirect) {
			HttpServletResponse hResponse = (HttpServletResponse)response;
			hResponse.sendRedirect(hRequest.getContextPath() + "/accountLoginForm.do");
		} else {
